...
public class ModelTraceExtractor implements ITraceExtractor {

	private modelTrace.SpecificTrace traceRoot;

	final private List<List<? extends modelTrace.States.Value>> valueTraces = Collections
			.synchronizedList(new ArrayList<>());

	private List<modelTrace.States.State> statesTrace;

	final private Map<modelTrace.Steps.SpecificStep, Integer> stepToStartingIndex = new HashMap<>();
	final private Map<modelTrace.Steps.SpecificStep, Integer> stepToEndingIndex = new HashMap<>();
	...
	private final List<modelTrace.States.Value> observedValues = new ArrayList<>();

	public ModelTraceExtractor() {
	...
	private ValueWrapper getValueWrapper(modelTrace.States.Value value, int valueIndex) {
		List<modelTrace.States.State> states = value.getStatesNoOpposite();
		modelTrace.States.State firstState = states.get(0);
	...
	private modelTrace.States.Value getValueAt(int traceIndex, int stateIndex) {
		modelTrace.States.Value result = null;
		if (traceIndex >= 0 && traceIndex < valueTraces.size()) {
			final List<? extends modelTrace.States.Value> valueTrace = valueTraces.get(traceIndex);
			final modelTrace.States.State state = statesTrace.get(stateIndex);
	...
	public ValueWrapper getValueWrapper(int traceIndex, int stateIndex) {
		modelTrace.States.Value value = getValueAt(traceIndex, stateIndex);
		if (value == null) {
			return null;
		}
		List<modelTrace.States.State> states = value.getStatesNoOpposite();
	...
	private List<Integer> computeStateComparisonList(List<modelTrace.States.Value> values) {
		final List<Integer> valueIndexes = new ArrayList<>();
		for (int i = 0; i < values.size(); i++) {
			final modelTrace.States.Value value = values.get(i);
			int idx = -1;
	...
	private void updateEquivalenceClasses(modelTrace.States.State state) {
		final List<modelTrace.States.Value> values = getAllStateValues(state, true);
	...
	private void updateEquivalenceClasses(List<modelTrace.States.State> states) {
	...
	public boolean compareStates(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final modelTrace.States.State state1;
		final modelTrace.States.State state2;

		if (eObject1 instanceof modelTrace.States.State) {
			state1 = (modelTrace.States.State) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof modelTrace.States.State) {
			state2 = (modelTrace.States.State) eObject2;
		} else {
			return false;
		}

		final List<modelTrace.States.Value> values1 = getAllStateValues(state1);
		final List<modelTrace.States.Value> values2 = getAllStateValues(state2);
		...
		for (int i = 0; i < values1.size(); i++) {
			if (!respectIgnored || !isValueTraceIgnored(i)) {
				final modelTrace.States.Value value1 = values1.get(i);
				final modelTrace.States.Value value2 = values2.get(i);
				if (value1 != value2) {
	...
	public boolean compareSteps(EObject eObject1, EObject eObject2) {
		final modelTrace.Steps.SpecificStep step1;
		final modelTrace.Steps.SpecificStep step2;

		if (eObject1 instanceof modelTrace.Steps.SpecificStep) {
			step1 = (modelTrace.Steps.SpecificStep) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof modelTrace.Steps.SpecificStep) {
			step2 = (modelTrace.Steps.SpecificStep) eObject2;
		} else {
			return false;
		}
	...
	public boolean compareStatesWithSteps(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		...
		if (compareStates(state1, state2, respectIgnored)) {
			final List<modelTrace.Steps.SpecificStep> endedSteps1 = state1.getEndedSteps();
			final List<modelTrace.Steps.SpecificStep> startedSteps1 = state1.getStartedSteps();
			final List<modelTrace.Steps.SpecificStep> endedSteps2 = state2.getEndedSteps();
			final List<modelTrace.Steps.SpecificStep> startedSteps2 = state2.getStartedSteps();
			if (endedSteps1.size() == endedSteps2.size() && startedSteps1.size() == startedSteps2.size()) {
				boolean result = true;
				for (int i = 0; i < endedSteps1.size(); i++) {
					final modelTrace.Steps.SpecificStep endedStep1 = endedSteps1.get(i);
					final modelTrace.Steps.SpecificStep endedStep2 = endedSteps2.get(i);
				...
				for (int i = 0; i < startedSteps1.size(); i++) {
					final modelTrace.Steps.SpecificStep startedStep1 = startedSteps1.get(i);
					final modelTrace.Steps.SpecificStep startedStep2 = startedSteps2.get(i);
	...
	public boolean compareTraces(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final modelTrace.SpecificTrace trace1;
		final modelTrace.SpecificTrace trace2;

		if (eObject1 instanceof modelTrace.SpecificTrace) {
			trace1 = (modelTrace.SpecificTrace) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof modelTrace.SpecificTrace) {
			trace2 = (modelTrace.SpecificTrace) eObject2;
		} else {
			return false;
		}

		final List<modelTrace.States.State> states1 = trace1.getStatesTrace();
		final List<modelTrace.States.State> states2 = trace2.getStatesTrace();
		...
		for (int i = 0; i < states1.size(); i++) {
			final modelTrace.States.State state1 = states1.get(i);
			final modelTrace.States.State state2 = states2.get(i);
	...
	private List<modelTrace.States.Value> getAllStateValues(modelTrace.States.State state) {
		return getAllStateValues(state, false);
	}

	private List<modelTrace.States.Value> getAllStateValues(modelTrace.States.State state,
			boolean includeHiddenValues) {
		final List<modelTrace.States.Value> result = new ArrayList<>();
		for (int i = 0; i < valueTraces.size(); i++) {
			if (includeHiddenValues || !isValueTraceIgnored(i)) {
				final List<? extends modelTrace.States.Value> trace = valueTraces.get(i);
				boolean notFound = true;
				for (modelTrace.States.Value value : trace) {
					if (value.getStatesNoOpposite().contains(state)) {
						result.add(value);
						notFound = false;
						break;
	...
	private boolean isStateBreakable(modelTrace.States.State state) {
		final boolean b = state.getStartedSteps().size() == 1;
		if (b) {
			modelTrace.Steps.SpecificStep s = state.getStartedSteps().get(0);
			return !(s instanceof modelTrace.Steps.Model_FSM_Run_ImplicitStep
					|| s instanceof modelTrace.Steps.Model_State_Step_ImplicitStep);
		}
		return true;
	}
	...
	public StateWrapper getStateWrapper(EObject state) {
		if (state instanceof modelTrace.States.State) {
			final int idx = statesTrace.indexOf(state);
			if (idx != -1) {
				final modelTrace.States.State state_cast = (modelTrace.States.State) state;
	...
	public List<StateWrapper> getStateWrappers(int start, int end) {
		...
		for (int i = startStateIndex; i < endStateIndex + 1; i++) {
			final modelTrace.States.State state = statesTrace.get(i);
	...
	public List<ValueWrapper> getValueWrappers(int valueTraceIndex, int start, int end) {
		...
		if (valueTraceIndex < valueTraces.size()) {
			final List<? extends modelTrace.States.Value> valueTrace = valueTraces.get(valueTraceIndex);
			for (modelTrace.States.Value value : valueTrace) {
	...
	public StepWrapper getStepWrapper(Step step) {
		if (step instanceof modelTrace.Steps.SpecificStep) {
			final modelTrace.Steps.SpecificStep step_cast = (modelTrace.Steps.SpecificStep) step;
			...
			if (step_cast instanceof SequentialStep<?>) {
				subSteps.addAll(((SequentialStep<modelTrace.Steps.SpecificStep>) step_cast).getSubSteps());
			}
	...
	public int getValuesTraceLength(int traceIndex) {
		if (traceIndex > -1 && traceIndex < valueTraces.size()) {
			List<? extends modelTrace.States.Value> trace = valueTraces.get(traceIndex);
			return trace.size();
		}
		return -1;
	}
	...
	public void statesAdded(List<EObject> states) {
		updateEquivalenceClasses(states.stream().map(e -> (modelTrace.States.State) e).collect(Collectors.toList()));
		notifyListeners();
	}
